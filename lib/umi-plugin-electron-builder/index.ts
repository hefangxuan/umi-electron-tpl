...
export default function (api: IApi) {
  ...
  if (isUpdateRootPkg) {
    ...
    fse.writeFileSync(path.join(process.cwd(), 'package.json'), JSON.stringify(rootPkg, null, 2));
  }
  ...
  api.modifyConfig((config) => {
    const { outputDir, externals, routerMode } = config.electronBuilder as ElectronBuilder;
  ...
  api.onBuildComplete(({ err }) => {
    ...
    if (err == null) {
      ...
      const buildDependencies = ['source-map-support', 'electron-devtools-installer'];
...
async function runInDevMode(api: IApi) {
  ...
  const args =
    electronArgs != null && electronArgs.length > 0
      ...
      : [`--inspect=${await getFreePort('127.0.0.1', 5858)}`];
...
async function runInMainBuild(api: IApi) {
  const { outputDir, mainWebpackConfig } = api.config.electronBuilder as ElectronBuilder;
  const absOutputDir = path.join(api.cwd, outputDir);
...
async function startMainDevWatch(api: IApi, hmrServer: HmrServer) {
  const { mainWebpackConfig } = api.config.electronBuilder as ElectronBuilder;
